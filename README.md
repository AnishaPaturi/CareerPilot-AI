...
* **Resume Auto-Apply Matcher**: Allows students to automatically match and apply to all eligible and skill-fit campus placement drives in one click. Features an integrated matching heuristic screening CGPA, branch, and role-specific technical skills.
...
* **Auto-Apply Integration**: Integrated a checkbox toggle (*"Automatically apply to matching campus drives after analysis"*) that automatically submits eligible applications for matching placement drives immediately following resume analysis.
...
* **Interviewer Simulator**: Conducts speech-based interviews using the browser's native **Web Speech API** for voice-to-text. Features live recording indicators, robust fallback checking for `getUserMedia` support (requires HTTPS/localhost), webcam stream preview, and graceful error clearing when inputs (mic/camera) are toggled off by the user in [InterviewRoom.jsx](file:///C:/Users/anish/OneDrive/College/Projects/AI-CareerOS/frontend/src/pages/Interview/InterviewRoom.jsx).
...
* **Interactive LeetCode Question Links**: Automatically matches common LeetCode problem titles to prepend their respective question numbers (e.g., `#1 Two Sum (Easy)`) if omitted by the AI, and renders titles as clickable links directly to their LeetCode problem page in [DSAPlanner.jsx](file:///C:/Users/anish/OneDrive/College/Projects/AI-CareerOS/frontend/src/components/DSAPlanner.jsx).
